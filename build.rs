...
fn main() {
    let result = Command::new("git").args(["rev-parse", "--short", "HEAD"]).output();
    ...
    let dirty = if let Ok(output) = result {
        if !String::from_utf8(output.stdout).unwrap_or_default().contains("modified:") {""} else {"+"}
    } else {
        " Error calling 'git'"
    };
    println!("cargo:rustc-env=GIT_HASH={}{}", git_hash, String::from(dirty));
